module Monotone.ConstantPropagation
(
cp,
Lattice(..),
LatticeVal(..),
...
--We consider the lattice Var -> Z^T
--Bottom is defined as the empty Map
...
--For each variable in the maps we try to join their values
joinOp :: Lattice Int -> Lattice Int -> Lattice Int
joinOp lmap rmap = M.unionWith joinLattice lmap rmap

--Find the meet of two lattice values. Generally this is Top, unless
...
--Otherwise if foreach variable, elements in the left map are at least as precise as the elements in the right map,
...
--We only consider IAssigns to have an effect on the Lattice
