...
setMeet lmap rmap = 
        M.null lmap 
            || 
        foldr (\x y -> lMeet (fst x) (snd x) && y) --Check if the elements in each tuple 'meet'
            True 
...
transferFunction :: Stat' -> M.Map Var (Lattice Int) -> M.Map Var (Lattice Int)
transferFunction (IAssign' l var expr) st = M.adjust (\_ -> parseExp expr st) var st
